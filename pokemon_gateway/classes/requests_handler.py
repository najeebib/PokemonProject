...
class RequestsHandler:
    ...
    def get_pokemon_by_id(self, pokemon_id: int):
        response = requests.get(f"http://pokemon_api-mypokemonserver-1:5001/pokemon/{pokemon_id}")
    ...
    def get_pokemon_by_name(self, pokemon_name):
        response = requests.get(f"http://pokemon_api-mypokemonserver-1:5001/pokemon?pokemon_name={pokemon_name}")
        pokemon_properties = response.json()
    ...
    def add_pokemon(self, pokemon: Pokemon):
        response = requests.post("http://pokemon_api-mypokemonserver-1:5001/pokemon", json=pokemon.model_dump())
    ...
    def add_trainer(self, trainer: Trainer):
        response = requests.post("http://pokemon_api-mypokemonserver-1:5001/trainer", json=trainer.model_dump())
        return response.json()
    # add pokemon to trainer  
    def add_pokemon_to_trainer(self, trainer_name: str, pokemon_name: str):
        response = requests.post(f"http://pokemon_api-mypokemonserver-1:5001/trainer/pokemon?trainer_name={trainer_name}&pokemon_name={pokemon_name}")
        return response.json()
    # delete pokemon from trainer
    def delete_pokemon_from_trainer(self,trainer_name: str, pokemon_name: str):
        response = requests.delete(f"http://pokemon_api-mypokemonserver-1:5001/trainer/pokemon?trainer_name={trainer_name}&pokemon_name={pokemon_name}")
        return response.json()
    # evolve the pokemon of a certain trainer
    def evolution(self,trainer_name: str, pokemon_name: str, next_evolution: str):
        next_pokemon = requests.get(f"http://pokemon_api-mypokemonserver-1:5001/pokemon?pokemon_name={next_evolution}")
        ...
        response = requests.put(f"http://pokemon_api-mypokemonserver-1:5001/evolution?trainer_name={trainer_name}&pokemon_name={pokemon_name}&next_evolution={next_evolution}")
        return response.json()
    ...
    def data_migration(self):
        response = requests.get("http://pokemon_api-mypokemonserver-1:5001/migration")
